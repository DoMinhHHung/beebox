...
| 22 | OpenAPI and Go SDK track the public headless surface | PASS | `api/openapi/openapi_test.go`, `api/openapi/phase2_self_service_test.go`, feature-specific OpenAPI tests, `sdk/go` feature clients/tests including one-call email-link completion coverage, and CI contract gates. Hosted HTML is intentionally not a second OpenAPI contract. |
