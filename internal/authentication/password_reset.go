...
type PasswordResetCodeHash struct{ encoded string }
func (h PasswordResetCodeHash) StorageEncoding() string { return h.encoded }
func (h PasswordResetCodeHash) Valid() bool { _, err := ParsePasswordHash(h.encoded); return err == nil }
func ParsePasswordResetCodeHash(encoded string) (PasswordResetCodeHash, error) {
	if _, err := ParsePasswordHash(encoded); err != nil { return PasswordResetCodeHash{}, ErrPasswordResetPersistence }
...
func GeneratePasswordResetCode() (string, error) {
	...
	if err != nil { return "", ErrPasswordResetPersistence }
	return fmt.Sprintf("%08d", n.Int64()), nil
}

func validPasswordResetCode(code string) bool {
	if len(code) != 8 { return false }
	for i := 0; i < len(code); i++ { if code[i] < '0' || code[i] > '9' { return false } }
	return true
}

func HashPasswordResetCode(code string) (PasswordResetCodeHash, error) { return HashPasswordResetCodeContext(context.Background(), code) }
func HashPasswordResetCodeContext(ctx context.Context, code string) (PasswordResetCodeHash, error) {
	if !validPasswordResetCode(code) { return PasswordResetCodeHash{}, ErrInvalidPasswordResetCode }
	hash, err := HashPasswordContext(ctx, []byte(code))
	if err != nil { return PasswordResetCodeHash{}, err }
	return PasswordResetCodeHash{encoded: hash.StorageEncoding()}, nil
}

func VerifyPasswordResetCode(hash PasswordResetCodeHash, code string) error { return VerifyPasswordResetCodeContext(context.Background(), hash, code) }
func VerifyPasswordResetCodeContext(ctx context.Context, hash PasswordResetCodeHash, code string) error {
	if !validPasswordResetCode(code) { return ErrInvalidPasswordResetCode }
...
type PasswordResetDelivery interface { DeliverPasswordResetCode(context.Context, string, string, time.Time) error }
...
type PasswordResetIssueResult struct { ShouldSend bool; Destination string; ExpiresAt time.Time }
...
type PasswordResetService struct { persistence PasswordResetPersistence; delivery PasswordResetDelivery }
func NewPasswordResetService(persistence PasswordResetPersistence, delivery PasswordResetDelivery) *PasswordResetService { return &PasswordResetService{persistence: persistence, delivery: delivery} }
...
func (s *PasswordResetService) RequestWithCorrelation(ctx context.Context, appID applicationinstance.InternalID, rawEmail string, correlationID audit.CorrelationID) error {
	if s == nil || s.persistence == nil || s.delivery == nil || !appID.Valid() || correlationID == (audit.CorrelationID{}) { return ErrPasswordResetPersistence }
	admission, ok := s.persistence.(PasswordResetAdmission)
	if !ok { return ErrPasswordResetPersistence }
	email, err := identity.NormalizeEmail(rawEmail)
	if err != nil { return identity.ErrInvalidEmail }
	...
	if err := admission.AllowPasswordResetIssue(ctx, appID, fingerprint); err != nil {
		if errors.Is(err, ErrPublicRateLimited) { return nil }
		return err
	}
	code, err := GeneratePasswordResetCode()
	if err != nil { return err }
	codeHash, err := HashPasswordResetCodeContext(ctx, code)
	if errors.Is(err, ErrKDFAdmissionLimited) { return nil }
	if err != nil { return err }
	if err := ctx.Err(); err != nil { return err }
	result, err := s.persistence.IssuePasswordReset(ctx, PasswordResetIssue{ApplicationInstanceID: appID, NormalizedEmail: email.ComparisonKey, CodeHash: codeHash, CorrelationID: correlationID})
	if err != nil {
		if errors.Is(err, ErrPasswordResetRateLimited) || errors.Is(err, ErrPasswordResetFailed) { return nil }
		return err
	}
	if !result.ShouldSend { return nil }
...
func (s *PasswordResetService) ConfirmWithCorrelation(ctx context.Context, appID applicationinstance.InternalID, rawEmail, code, newPassword string, correlationID audit.CorrelationID) error {
	if s == nil || s.persistence == nil || !appID.Valid() || correlationID == (audit.CorrelationID{}) { return ErrPasswordResetPersistence }
	if !validPasswordResetCode(code) { return ErrInvalidPasswordResetCode }
	admission, ok := s.persistence.(PasswordResetAdmission)
	if !ok { return ErrPasswordResetPersistence }
	email, err := identity.NormalizeEmail(rawEmail)
	...
	if err := admission.AllowPasswordResetConfirm(ctx, appID, fingerprint); err != nil {
		if errors.Is(err, ErrPublicRateLimited) { return ErrPasswordResetRateLimited }
		return err
	}
	if err := ctx.Err(); err != nil { return err }
	snapshot, err := s.persistence.LoadPasswordReset(ctx, appID, email.ComparisonKey)
	if err != nil {
		if errors.Is(err, ErrPasswordResetFailed) { return ErrPasswordResetFailed }
		return err
	}
	verifyErr := VerifyPasswordResetCodeContext(ctx, snapshot.CodeHash, code)
	if errors.Is(verifyErr, ErrKDFAdmissionLimited) { return ErrPasswordResetRateLimited }
	matched := verifyErr == nil
	if verifyErr != nil && !errors.Is(verifyErr, ErrPasswordResetFailed) { return verifyErr }
	if err := ctx.Err(); err != nil { return err }

	var newHash PasswordHash
	if matched {
		newHash, err = HashPasswordContext(ctx, prepared)
		if errors.Is(err, ErrKDFAdmissionLimited) { return ErrPasswordResetRateLimited }
		if err != nil { return err }
	}
	err = s.persistence.FinalizePasswordReset(ctx, PasswordResetFinalize{ApplicationInstanceID: appID, EmailIdentifierID: snapshot.EmailIdentifierID, UserID: snapshot.UserID, ChallengeGeneration: snapshot.ChallengeGeneration, CredentialGeneration: snapshot.CredentialGeneration, Matched: matched, NewPasswordHash: newHash, CorrelationID: correlationID})
	if err != nil { return err }
	if !matched { return ErrPasswordResetFailed }
	return nil
}
