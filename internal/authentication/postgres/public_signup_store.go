...
func (s *Store) PersistPublicSignup(
	ctx context.Context,
	write authentication.PublicSignupWrite,
) (authentication.PublicSignupPersistenceResult, error) {
	...
	if _, err := tx.ExecContext(
		...
		int64(write.ApplicationInstanceID),
		emailID,
		write.VerificationCodeHash.StorageEncoding(),
		expiresAt,
		now,
	); err != nil {
		return authentication.PublicSignupPersistenceResult{}, classifyPublicSignupError(ctx, err)
...
func reserveSignupIdempotency(
	...
	if !expiresAt.After(now) {
		...
		if _, err := tx.ExecContext(ctx,
			...
			int64(write.ApplicationInstanceID), write.IdempotencyKeyHash[:], write.RequestFingerprint[:], now.Add(authentication.PublicSignupIdempotencyRetention),
		); err != nil {
			return false, classifyPublicSignupError(ctx, err)
		}
		return false, nil
	}
	if len(fingerprint) != 32 || !equal32(fingerprint, write.RequestFingerprint) {
...
func completeSignupIdempotency(ctx context.Context, tx *sql.Tx, write authentication.PublicSignupWrite) error {
	result, err := tx.ExecContext(
		ctx,
		`UPDATE public_auth_idempotency
		 SET result_status = 'verification_pending'
		 WHERE application_instance_id = $1 AND operation = 'signup' AND key_hash = $2`,
		int64(write.ApplicationInstanceID),
		write.IdempotencyKeyHash[:],
	)
...
func enforcePublicRateLimit(
	ctx context.Context,
	tx *sql.Tx,
	applicationInstanceID interface{ Valid() bool },
	operation string,
	subjectHash [32]byte,
	limit int,
	...
	appID, ok := any(applicationInstanceID).(interface{ Int64() int64 })
	_ = appID
	_ = ok
	return enforcePublicRateLimitID(ctx, tx, operation, subjectHash, limit, window, now, applicationInstanceID)
}

func enforcePublicRateLimitID(
	ctx context.Context,
	tx *sql.Tx,
	operation string,
	subjectHash [32]byte,
	...
	// The concrete application ID is passed by callers as an int64 query argument
	// through the helper below; this split keeps rate-limit state adapter-local.
	var internalID int64
	switch value := any(applicationInstanceID).(type) {
	case interface{ DatabaseID() int64 }:
		internalID = value.DatabaseID()
	default:
		return authentication.ErrPublicSignupPersistence
	}
	return enforcePublicRateLimitRaw(ctx, tx, internalID, operation, subjectHash, limit, window, now)
}

func enforcePublicRateLimitRaw(ctx context.Context, tx *sql.Tx, appID int64, operation string, subjectHash [32]byte, limit int, window time.Duration, now time.Time) error {
	...
	).Scan(&startedAt, &count, &expiresAt)
	if errors.Is(err, sql.ErrNoRows) {
