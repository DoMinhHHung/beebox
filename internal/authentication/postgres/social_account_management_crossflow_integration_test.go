...
func TestSocialLinkFinalizeAndUnlinkConvergeWithoutResurrection(t *testing.T) {
	...
	go func() {
		...
		errs <- store.FinalizeSocialLink(ctx, authentication.SocialLinkFinalize{AttemptID: attempt.AttemptID, ProviderSubject: "same-subject", CorrelationID: correlation})
	}()
	go func() {
		defer wg.Done()
		<-start
	...
	if !unlinkSucceeded {
		t.Fatal("unlink did not complete successfully")
	}
...
func TestSocialLinkAttemptCreationAndUnlinkHaveSerializableOutcome(t *testing.T) {
	...
	var wg sync.WaitGroup
	wg.Add(2)
	go func() { defer wg.Done(); <-start; errs <- store.CreateSocialLinkAttempt(ctx, write) }()
	go func() {
		defer wg.Done()
		<-start
		correlation, _ := audit.NewCorrelationID()
...
func TestSocialProofAndUnlinkNeverReauthenticateFormerOwner(t *testing.T) {
	...
	var wg sync.WaitGroup
	wg.Add(2)
	go func() { defer wg.Done(); <-start; errs <- store.FinalizeSocialProof(ctx, proof) }()
	go func() {
		defer wg.Done()
		<-start
		correlation, _ := audit.NewCorrelationID()
	...
	if err == nil && owner.Int64 == int64(user.InternalID) {
		t.Fatalf("provider subject resurrected former owner user_id=%d", owner.Int64)
	}
...
func TestSocialCompletionExchangeAndUnlinkLinearize(t *testing.T) {
	...
	start := make(chan struct{})	type result struct { kind string; err error }
	results := make(chan result, 2)
	var wg sync.WaitGroup
	wg.Add(2)
	go func() { defer wg.Done(); <-start; _, err := store.ExchangeSocialCompletion(ctx, final); results <- result{"exchange", err} }()
	go func() { defer wg.Done(); <-start; correlation, _ := audit.NewCorrelationID(); results <- result{"unlink", store.UnlinkSocialAccount(ctx, current, target, availability, correlation)} }()
	close(start)
	wg.Wait()
	close(results)
	var exchangeErr error
	for result := range results {
		if result.kind == "unlink" && result.err != nil { t.Fatalf("unlink error=%v", result.err) }
		if result.kind == "exchange" { exchangeErr = result.err }
	}
	if exchangeErr != nil && !errors.Is(exchangeErr, authentication.ErrSocialCompletionInvalid) {
		t.Fatalf("exchange error=%v", exchangeErr)
	}
	var newSessions int
	if err := db.QueryRowContext(ctx, `SELECT count(*) FROM sessions WHERE public_id=$1 AND revoked_at IS NULL`, newSession).Scan(&newSessions); err != nil { t.Fatal(err) }
	if exchangeErr == nil && newSessions != 1 { t.Fatalf("successful exchange sessions=%d", newSessions) }
	if errors.Is(exchangeErr, authentication.ErrSocialCompletionInvalid) && newSessions != 0 { t.Fatalf("invalid exchange created sessions=%d", newSessions) }
...
func TestLastMethodDenialPreservesPendingSecurityStateAndAuditIsMinimized(t *testing.T) {
	...
	if err := store.CreateSocialLinkAttempt(ctx, authentication.SocialLinkAttemptWrite{ApplicationInstanceID: app.InternalID, UserID: user.InternalID, SessionPublicID: sessionID, Provider: authentication.ProviderGitHub, CanonicalRedirectURL: "https://app.example.test/link-complete", StateHash: state, RecentAuthAt: created, ProviderPKCECiphertext: ciphertext, CreatedAt: time.Now().UTC(), ExpiresAt: created.Add(authentication.SocialLinkFreshness)}); err != nil { t.Fatal(err) }
	...
	if !errors.Is(err, authentication.ErrLastAuthenticationMethod) { t.Fatalf("unlink error=%v", err) }
	...
	if err := db.QueryRowContext(ctx, `SELECT consumed_at FROM social_auth_completion_grants WHERE code_hash=$1`, completion[:]).Scan(&consumed); err != nil || consumed.Valid { t.Fatalf("grant consumed=%v err=%v", consumed, err) }
	var appID, actorID, subjectID int64
	var action, reference, outcome string
	if err := db.QueryRowContext(ctx, `SELECT application_instance_id,actor_user_id,subject_user_id,action,resource_reference,outcome FROM audit_events WHERE action=$1 AND correlation_id=$2`, audit.ActionSocialUnlinkDenied, correlation[:]).Scan(&appID, &actorID, &subjectID, &action, &reference, &outcome); err != nil { t.Fatal(err) }
	...
	if reference == "secret-provider-subject" { t.Fatal("audit leaked provider subject") }
}
...
func TestSocialUnlinkAuditFailureRollsBackAllSuccessSideMutation(t *testing.T) {
	...
	mustExec(t, ctx, db, `INSERT INTO social_auth_completion_grants(application_instance_id,user_id,code_hash,client_code_challenge,expires_at) VALUES($1,$2,$3,$4,CURRENT_TIMESTAMP+INTERVAL '5 minutes')`, int64(app.InternalID), int64(user.InternalID), completion[:], crossFlowChallenge)
	mustExec(t, ctx, db, `CREATE FUNCTION reject_social_unlink_success_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.action = 'authentication.social.unlink_succeeded' THEN RAISE EXCEPTION 'reject unlink audit'; END IF; RETURN NEW; END $$`)
	mustExec(t, ctx, db, `CREATE TRIGGER reject_social_unlink_success_audit_trigger BEFORE INSERT ON audit_events FOR EACH ROW EXECUTE FUNCTION reject_social_unlink_success_audit()`)
	correlation, _ := audit.NewCorrelationID()
	...
	if !errors.Is(err, authentication.ErrSocialAccountPersistence) { t.Fatalf("unlink error=%v", err) }
	assertIdentityExists(t, ctx, db, target, true)
	var canceled sql.NullTime
	var gotCiphertext []byte
	if err := db.QueryRowContext(ctx, `SELECT canceled_at,provider_pkce_ciphertext FROM social_link_attempts WHERE state_hash=$1`, state[:]).Scan(&canceled, &gotCiphertext); err != nil || canceled.Valid || !reflect.DeepEqual(gotCiphertext, ciphertext) { t.Fatalf("attempt rollback canceled=%v ciphertext=%x err=%v", canceled, gotCiphertext, err) }
	var consumed sql.NullTime
	if err := db.QueryRowContext(ctx, `SELECT consumed_at FROM social_auth_completion_grants WHERE code_hash=$1`, completion[:]).Scan(&consumed); err != nil || consumed.Valid { t.Fatalf("grant rollback consumed=%v err=%v", consumed, err) }
}
