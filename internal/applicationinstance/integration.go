...
type IntegrationService struct { persistence IntegrationPersistence }
func NewIntegrationService(p IntegrationPersistence) *IntegrationService { return &IntegrationService{persistence:p} }
...
func (s *IntegrationService) ResolvePublishable(ctx context.Context, key string) (Instance, error) {
	if !validPublishableKey(key) || s == nil || s.persistence == nil { return Instance{}, ErrInvalidCredential }
...
func (s *IntegrationService) AuthenticateSecret(ctx context.Context, key string) (Instance, error) {
	...
	if !ok || s == nil || s.persistence == nil { return Instance{}, ErrInvalidCredential }
	...
	if err == nil && len(storedHash) == 32 { compare = subtle.ConstantTimeCompare(storedHash, candidate[:]) }
	if err != nil || compare != 1 { return Instance{}, ErrInvalidCredential }
	if credential.RevokedAt != nil { return Instance{}, ErrCredentialRevoked }
...
func newCredentialMaterial(kind CredentialKind) (CredentialMaterial, string, error) {
	publicID, err := publicid.NewUUIDv4("cred"); if err != nil { return CredentialMaterial{}, "", ErrInvalidCredential }
	...
	case CredentialKindPublishable:
		keyID, err := publicid.NewUUIDv4("pk"); if err != nil { return CredentialMaterial{}, "", ErrInvalidCredential }
	...
	case CredentialKindSecret:
		var secret [32]byte; if _, err := rand.Read(secret[:]); err != nil { return CredentialMaterial{}, "", ErrInvalidCredential }
		hash := sha256.Sum256(secret[:]); material.SecretHash = append([]byte(nil), hash[:]...)
...
func parseSecretKey(key string) (string, []byte, bool) {
	if !strings.HasPrefix(key, "bb_sk_") { return "", nil, false }
	parts := strings.Split(strings.TrimPrefix(key,"bb_sk_"), "."); if len(parts) != 2 { return "", nil, false }
	if !publicid.IsUUIDv4("cred_"+parts[0], "cred") { return "", nil, false }
	secret, err := base64.RawURLEncoding.DecodeString(parts[1]); if err != nil || len(secret) != 32 { return "", nil, false }
