...
func (s *IntegrationStore) CreateCredential(ctx context.Context, appID applicationinstance.InternalID, kind applicationinstance.CredentialKind, material applicationinstance.CredentialMaterial) (applicationinstance.Credential, error) {
	...
	err := db.QueryRowContext(ctx, `INSERT INTO application_credentials (public_id,application_instance_id,kind,publishable_key,secret_hash)
		VALUES ($1,$2,$3,$4,$5)
...
func (s *IntegrationStore) RevokeCredential(ctx context.Context, id applicationinstance.CredentialPublicID) error {
	if err := ctx.Err(); err != nil { return err }
	if s==nil || s.pool==nil || !id.Valid() { return applicationinstance.ErrInvalidCredential }
	db:=s.pool.OpenSQLDB(); defer db.Close(); result,err:=db.ExecContext(ctx,`UPDATE application_credentials SET revoked_at=COALESCE(revoked_at,CURRENT_TIMESTAMP) WHERE public_id=$1`,string(id)); if err!=nil { return classifyIntegrationError(ctx,err) }; n,_:=result.RowsAffected(); if n==0 { return applicationinstance.ErrCredentialNotFound }; return nil
}
...
func (s *IntegrationStore) AddAllowedOrigin(ctx context.Context, appID applicationinstance.InternalID, canonical string) (applicationinstance.AllowedOrigin,error) {
	...
	db:=s.pool.OpenSQLDB(); defer db.Close(); var o applicationinstance.AllowedOrigin; var app int64
	err:=db.QueryRowContext(ctx,`INSERT INTO application_allowed_origins (application_instance_id,canonical_origin) VALUES ($1,$2) RETURNING id,application_instance_id,canonical_origin,created_at`,int64(appID),canonical).Scan(&o.InternalID,&app,&o.CanonicalOrigin,&o.CreatedAt)
	if err!=nil{return applicationinstance.AllowedOrigin{},classifyIntegrationError(ctx,err)}; o.ApplicationInstanceID=applicationinstance.InternalID(app); o.CreatedAt=o.CreatedAt.UTC(); return o,nil
}
