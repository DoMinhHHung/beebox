...
func TestTOTPRoundTripAADTamperAndRotation(t *testing.T) {
	...
	if _, err := oldRing.DecryptTOTP(ctx, tampered); !errors.Is(err, ErrDecrypt) {
	...
	for name, wrong := range map[string]Context{
		...
		if _, err := oldRing.DecryptTOTP(wrong, a); !errors.Is(err, ErrDecrypt) {
			t.Fatalf("wrong %s AAD accepted: %v", name, err)
		}
	}
	badVersion := a
	badVersion.Version = 2
	if _, err := oldRing.DecryptTOTP(ctx, badVersion); !errors.Is(err, ErrDecrypt) {
		t.Fatalf("unknown version accepted: %v", err)
	}
	badKey := a
	badKey.KeyID = "missing"
	if _, err := oldRing.DecryptTOTP(ctx, badKey); !errors.Is(err, ErrDecrypt) {
	...
	if _, err := rotated.DecryptTOTP(ctx, a); err != nil {
		t.Fatalf("old key stopped decrypting after rotation: %v", err)
	}
	...
	if fresh.KeyID != "new" {
		t.Fatalf("new encryption used key %q", fresh.KeyID)
	}
}
...
func TestErrorsDoNotContainSecretMaterial(t *testing.T) {
	...
	if bytes.Contains([]byte(err.Error()), []byte(secretValue)) {
		t.Fatal("config error leaked key material")
	}
}
