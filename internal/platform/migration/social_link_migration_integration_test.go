...
func TestSocialLinkMigrationUpgradesVersion15AndEnforcesBindings(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if entry.Name() != "00016_social_account_linking.sql" && entry.Name() != "00017_social_account_management.sql" && entry.Name() != "00018_passkeys.sql" && entry.Name() != "00019_totp_mfa.sql" {
