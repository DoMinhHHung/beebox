...
func TestEmbeddedSourcesAreValidAndOrdered(t *testing.T) {
	...
	if err != nil { t.Fatalf("fs.Sub() error = %v", err) }
	if err := validateSources(sources); err != nil { t.Fatalf("validateSources() error = %v", err) }
	entries, err := fs.ReadDir(sources, ".")
	if err != nil { t.Fatalf("fs.ReadDir() error = %v", err) }
	want := []string{
	...
	if len(entries) != len(want) { t.Fatalf("embedded migration count = %d, want %d", len(entries), len(want)) }
	for i, name := range want { if entries[i].Name() != name { t.Fatalf("migration[%d] = %q, want %q", i, entries[i].Name(), name) } }
}

func TestValidateSourcesRejectsUnsafeOrInvalidSources(t *testing.T) {
	tests := []struct { name string; sources fs.FS }{
	...
	for _, tt := range tests { t.Run(tt.name, func(t *testing.T) { err := validateSources(tt.sources); if !errors.Is(err, errInvalidSources) { t.Fatalf("validateSources() error = %v, want stable invalid-sources error", err) } }) }
}

func TestUpRequiresDeadlineAndClosesAdapterWithSafeError(t *testing.T) {
	db, closed := openTestDB(t)
	if err := db.Ping(); err != nil { t.Fatalf("test db Ping() error = %v", err) }
	err := upWithSources(context.Background(), db, fstest.MapFS{"00001_baseline.sql":{Data:[]byte(validMigration)}})
	if !errors.Is(err, errDeadlineRequired) { t.Fatalf("upWithSources() error = %v, want deadline-required error", err) }
	if strings.Contains(err.Error(), "provider") || strings.Contains(err.Error(), "dsn") { t.Fatalf("upWithSources() error leaks provider detail: %q", err) }
	if err := db.Ping(); err == nil || err.Error() != "sql: database is closed" { t.Fatalf("db.Ping() after Up = %v, want closed adapter", err) }
	if got := closed.Load(); got != 1 { t.Fatalf("underlying adapter Close() calls = %d, want 1", got) }
}

type unsortedFS struct { fstest.MapFS; order []string }
func (u unsortedFS) ReadDir(name string) ([]fs.DirEntry,error) { if name!="." { return nil,fs.ErrNotExist }; entries:=make([]fs.DirEntry,0,len(u.order)); for _,path:=range u.order { info,err:=fs.Stat(u.MapFS,path); if err!=nil{return nil,err}; entries=append(entries,fs.FileInfoToDirEntry(info)) }; return entries,nil }
